...
@implementation KJFFmpegPlayer

+ (KJFFmpegPlayer *)sharedInstance {
  static KJFFmpegPlayer *instance;
  static dispatch_once_t onceToken;
  dispatch_once(&onceToken, ^{
    instance = [[KJFFmpegPlayer alloc] init];
    [instance initFFmpeg];
  });
  return instance;
}
...
#pragma mark - video and audio player
...
//    is->resample_size = av_rescale_rnd(avresample_get_delay(is->pSwrCtx) + inframe->nb_samples,
//                                       44100,
//                                       44100,
//                                       AV_ROUND_UP);
//#else
//    is->resample_size = av_rescale_rnd(swr_get_delay(is->pSwrCtx, 44100) + inframe->nb_samples,
//                                       44100,
//                                       44100,
//                                       AV_ROUND_UP);
//#endif
...
//    }
//    
//    av_samples_alloc(&is->pResampledOut, &is->resample_lines, 2, is->resample_size, AV_SAMPLE_FMT_S16, 0);
//  }
//  
//#ifdef __LIBAVRESAMPLE__
//  
...
////      fprintf(stderr, "frame type: %s\n", (pFrame->pict_type==1 ? "I" :
////                                          (pFrame->pict_type==2 ? "P" : "B")));
////    }
//    
...
#pragma mark - sync video to audio
...
//int packet_queue_put(PacketQueue *q, AVPacket *pkt) {
//  
////  if (q == (&global_video_state->videoq)) {
////    if (pkt->dts != pkt->pts)
////      fprintf(stderr, "%d-%d\n", pkt->dts, pkt->pts);
////  }
//  
...
//}
//
//// put process picture frame into queue
//// ready to display with order in queue
...
// * => just a backup pts for we can get it if later frame don't have pts
// */
...
//  pFrame = av_frame_alloc();
//  
//  // waiting until avcodec_decode return a finsihed frame
//  for(;;) {
//    if(packet_queue_get(&is->videoq, packet, 1) < 0) {
//      // means we quit getting packets
//      break;
...
//    }
////    else {
////      fprintf(stderr, "frame finished. frame type: %d\n", pFrame->pict_type);
////      fprintf(stderr, "pending for needed frames. pts:%ld dts: %ld\n", packet->pts, packet->dts);
////    }
//    av_free_packet(packet);
//  }
//  av_free(pFrame);
...
#pragma mark - sync video and audio dynamic
...
#pragma mark - video/audio player with seek features
...
typedef struct VideoState {
  ...
  PacketQueue     videoq;
  VideoPicture    pictq[VIDEO_PICTURE_QUEUE_SIZE];
  int             pictq_size, pictq_rindex, pictq_windex; // size, read index, write index
  SDL_mutex       *pictq_mutex;
  SDL_cond        *pictq_cond;
  SDL_Thread      *parse_tid;
  SDL_Thread      *video_tid;
...
double get_master_clock(VideoState *is) {
  double clock = -1;
  if(is->av_sync_type == AV_SYNC_VIDEO_MASTER) {
    clock = get_video_clock(is);
  } else if(is->av_sync_type == AV_SYNC_AUDIO_MASTER) {
    clock = get_audio_clock(is);
  } else {
    clock = get_external_clock(is);
  }
...
long audio_tutorial_resample(VideoState *is, struct AVFrame *inframe) {
  ...
  if( is->pResampledOut == NULL || inframe->nb_samples > is->resample_size) {
    ...
    av_samples_alloc(&is->pResampledOut, &is->resample_lines, 2,
...
#else
  //AVResample NEW
  resample_nblen = avresample_convert(is->pSwrCtx, (uint8_t **)&is->pResampledOut, 0,
                                      (int)is->resample_size,
                                      (uint8_t **)resample_input_bytes, 0, inframe->nb_samples);
#endif
...
// TODO: this is not called, deprecated
// => find another callback
void our_release_buffer(struct AVCodecContext *c, AVFrame *pic) {
  if(pic) av_freep(&pic->opaque);
  avcodec_default_release_buffer(c, pic);
}
