...
- (int)readVideoInfoAndExtractSampleFramesWithFile:(NSString *)filePath {
  ...
  NSLog(@"video time base: numerator: %d - denominator: %d - todouble: %lf",
  ...
  while(av_read_frame(pFormatCtx, &packet)>=0) {
    ...
    if(packet.stream_index==videoStream) {
      ...
      //NSLog(@"before decode packet dst: %ld - pst: %ld", packet.dts, packet.pts);
      
      // Decode video frame
      avcodec_decode_video2(pCodecCtx, pFrame, &frameFinished,
                            &packet);
      
      // each DTS stamp is 512, so when compare with
      // timebase: 12288 = 512 * 24 (24 is fps)
      long pts = 0;
      ...
      //NSLog(@"after decode packet dst: %ld - pst: %ld", packet.dts, packet.pts);
